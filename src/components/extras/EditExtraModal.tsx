...
import { updateExtra, getExtraById } from "../../services/Extras.Service";
import { toast } from "react-toastify";
import { ExtraFormData } from "../../types";
import Spinner from "../Spinner";

export default function EditExtraModal() {
    const extraId = new URLSearchParams(useLocation().search).get("editExtra") || "";
    const show = new URLSearchParams(useLocation().search).has("editExtra");
    ...
    // Query para obtener los datos del extra
    const { data: extraData, isLoading: isLoadingExtra } = useQuery({
        queryKey: ["extra", extraId],
        queryFn: () => getExtraById({ extraId }),
        enabled: !!extraId && show,
        retry: false,
    });

    // Llenar el formulario cuando se cargan los datos del extra
    useEffect(() => {
        if (extraData) {
            setValue("name", extraData.name);
            setValue("price", extraData.price);
        }
    }, [extraData, setValue]);

    // Mutación para actualizar extra
    ...
    const handleFormSubmit = (data: ExtraFormData) => {
        ...
        mutate({ extraId, formData: formDataWithParsedPrice });
    };
    ...
    if (isLoadingExtra) return <Spinner />;
                                ...
                                <div className="relative bg-gradient-to-r from-orange-600 to-red-600 px-8 py-8 text-white">
                                    ...
                                    <div className="flex items-center space-x-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="mt-1 text-orange-100">Modifica la información de "{extraData?.name}"</p>
                                        </div>
                                    </div>
                                </div>
